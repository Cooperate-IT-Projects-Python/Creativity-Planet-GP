...
from profilesApp import models as profile
...
class ActiveCamps(models.Model):
    ...
    def clean(self):
        if self.end_date < self.start_date:
            raise ValidationError("end_date Cant be less Than start_date")
...
class CampsEnrollment(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='campsEnrollment')
    payed = models.BooleanField(default=False)
    max_attendees = models.IntegerField(default=1, validators=[MinValueValidator(1)])
    city = models.CharField(max_length=100,default="Cairo", null=False, blank=False)
    ...
    def __str__(self):
        return f"{self.user} Enrolled {self.camp.title} EnrollID {self.id}"
...
class QrCode(models.Model):
    ...
    camp = models.ForeignKey(CampsEnrollment, on_delete=models.CASCADE, related_name='qrCode')
...
class AttendeesNamesAges(models.Model):
    ...
    age = models.IntegerField(default=5, validators=[MinValueValidator(5), MaxValueValidator(12)])
